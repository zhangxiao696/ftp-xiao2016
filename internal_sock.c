...
void priv_sock_send_cmd(int fd, char cmd)
{
	...
	if(ret != sizeof(cmd))
	{
		fprintf(stderr, "priv_sock_cmd_error\n");
		exit(EXIT_FAILURE);
	}
}
...
char priv_sock_get_cmd(int fd)
{
	...
	if(ret == 0)
	{
		printf("a ftp process exit\n");
		exit(EXIT_SUCCESS);
	}
	if(ret != sizeof(res))
	{
		printf("priv_sock_get_cmd error\n");
		exit(EXIT_FAILURE);
	}
	return res;
}
...
char priv_sock_get_result(int fd)
{
	...
	if(ret == 0)
	{
		printf("b ftp process exit\n");
		exit(EXIT_FAILURE);
	}
	if(ret != sizeof(res))
	{
		printf("priv_sock_get_cmd_error\n");
		exit(EXIT_FAILURE);
	}
	return res;
}
...
void priv_sock_recv_buf(int fd, char *buf, unsigned int len)
{
	...
	if(recv_len > len)
	{
		fprintf(stderr, "priv_sock_get_buf error\n");
		exit(EXIT_FAILURE);
	}
